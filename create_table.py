...
def get_dipole_vectors(jffAtoms):
	...
	# Array containing positions of the CE2, CD2, CD1, and NE2 atoms
	...
	for j in range(int(len(atoms)/4)):
		v1s.append( normalize_vec(atoms[(4*j)+2] - midpoint(atoms[(4*j)+1], atoms[(4*j)+3])) )
		v2s.append( normalize_vec(atoms[(4*j)+3] - midpoint(atoms[(4*j)+1], atoms[(4*j)+3])) )
		vs_to_rot.append( normalize_vec(atoms[(4*j)] - midpoint(atoms[(4*j)+1], atoms[(4*j)+3])) )
	
	# Looping through vectors to calculate dipoles
	index = 0
	dipole_vectors = np.zeros((8,3))
